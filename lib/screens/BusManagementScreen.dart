    ...
    try {
      ...
      if (change > 0) {
        ...
        if (availableDrivers.isEmpty) {
          _showWarning(
            "لا يمكن إضافة حافلة جديدة: لا يوجد سائقون متاحون (غير معينين). الرجاء تسجيل حساب سائق جديد أولاً.",
          );
      ...
      await FirebaseFirestore.instance.runTransaction((transaction) async {
        ...
        if (change < 0) {
          ...
          if (totalStudents > maxPossibleCapacity) {
            _showWarning(
              "لا يمكن حذف الحافلة: عدد الطلاب ($totalStudents) يتجاوز سعة الحافلات المتبقية ($maxPossibleCapacity).",
            );
            return;
          }
        }
                  ...
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemBuilder: (itemContext, index) {
                        // ✅ Renamed to itemContext
                        final driverDoc = availableDrivers[index];
                        final data = driverDoc.data();

                        final firstName = data.containsKey('firstName')
                            ? data['firstName']
                            : '';
                        final lastName = data.containsKey('lastName')
                            ? data['lastName']
                            : '';
                        final driverName = "$firstName $lastName".trim();
                        final displayName = driverName.isEmpty
                            ? 'سائق ${index + 1}'
                            : driverName;

                        final driverPhone = data.containsKey('phone')
                            ? data['phone']
                            : driverDoc.id;
  ...
  Future<void> _confirmDelete() async {
    final bool? confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          "هل أنت متأكد من رغبتك في حذف حافلة؟ سيتم إعادة توزيع جميع الطلاب.",
          textAlign: TextAlign.right,
        ),
        actions: [
          TextButton(
            ...
            return StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, driverSnap) {
                ...
                if (driverSnap.hasData && driverSnap.data!.docs.isNotEmpty) {
                  final driverData =
                      driverSnap.data!.docs.first.data()
                          as Map<String, dynamic>;

                  final firstName = driverData.containsKey('firstName')
                      ? driverData['firstName']
                      : '';
                  final lastName = driverData.containsKey('lastName')
                      ? driverData['lastName']
                      : '';
                  final fullName = "$firstName $lastName".trim();

                  // Fallback to phone number if they didn't set a name
                  driverBadgeText = fullName.isNotEmpty
                      ? fullName
                      : (driverData['phone'] ?? 'سائق');
                }
