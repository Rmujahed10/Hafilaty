...
/// You can move this to a shared utils file later.
/// Same logic you used in ParentRegistrationScreen.
...
class DriverRegistrationScreen extends StatefulWidget {
  // optional: allow passing a success route like the parent screen
  ...
  Future<void> _handleRegistration() async {
    ...
    try {
      ...
      if (e.code == 'email-already-in-use') {
        ...
        debugPrint('FirebaseAuth Error (driver): ${e.code} - ${e.message}');
      }
      _showError(message);
      setState(() => _isLoading = false);
      return;
    } catch (e) {
      debugPrint('Unexpected error during Auth (driver): $e');
    ...
    if (user != null) {
      ...
      try {
        ...
        final Map<String, dynamic> userData = {
          'uid': uid,
          'role': 'driver', // 👈 مهم: تمييز السائقين
          'phone': phone,
          'firstName': _firstNameController.text.trim(),
          'lastName': _lastNameController.text.trim(),
          'nationalId': _idController.text.trim(),
          'licenseNumber': _licenseController.text.trim(),
          'birthDate': _birthDateController.text.trim(), // نص بسيط حالياً
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('تم تسجيل السائق بنجاح! يرجى تسجيل الدخول.')),
        );
        ...
        debugPrint(
            'Firestore write failed for driver UID $uid: ${e.code} - ${e.message}');
        _showError('تم إنشاء الحساب ولكن فشل حفظ بيانات السائق. يرجى المحاولة مرة أخرى.');

        // تنظيف حساب الـ Auth إذا فشل تخزين البيانات
        await user.delete();
            ...
            child: Column(
              children: [
                ...
                _buildLabeledTextField(
                  ...
                  keyboardType: TextInputType.number,
                  validator: (value) =>
                      value == null || value.isEmpty ? 'الرجاء إدخال رقم الرخصة' : null,
                ),
                const SizedBox(height: 20),

                // Birth date (simple text for now)
                _buildLabeledTextField(
