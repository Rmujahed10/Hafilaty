  ...
  Future<void> _loadTripData() async {
    try {
      ...
      final busDoc = await FirebaseFirestore.instance
          ...
          .get();
      if (busDoc.exists) {
        String statusField = widget.isMorningTrip
            ? 'morningTripStatus'
            : 'afternoonTripStatus';
  ...
  void _checkSpeeding(Position position) {
    ...
    if (speedKmh > speedLimit) {
      ...
      if (_lastSpeedingEventTime == null ||
          now.difference(_lastSpeedingEventTime!).inMinutes >= 3) {
        ...
        debugPrint(
          "🚨 Speeding event logged: ${speedKmh.toStringAsFixed(1)} km/h",
        );
      }
    }
  }
  ...
  void _showArrivalDialog(StudentPinModel student) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          "أنت الآن عند منزل الطالب: ${student.name}.\nالرجاء مسح الـ QR الخاص به.",
